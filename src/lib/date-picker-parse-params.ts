...
const CEREMONY_IDS = new Set(WEDDING_CEREMONIES.map((c) => c.id));
...
function parseCeremonyId(raw: string | undefined): WeddingCeremonyId {
  if (raw && CEREMONY_IDS.has(raw as WeddingCeremonyId)) {
    return raw as WeddingCeremonyId;
  }
  return DEFAULT_CEREMONY_ID;
}
...
export function datePickerInputFromSearchParams(
  sp: Record<string, string | string[] | undefined>,
): {
  submitted: boolean;
  ...
  if (!first(sp, "a_year")) {
    return {
      submitted: false,
      ceremonyId: parseCeremonyId(first(sp, "ceremony")),
  ...
  const ceremonyId = parseCeremonyId(first(sp, "ceremony"));
  const startDate = first(sp, "startDate") ?? defaults.startDate;
  const endDate = first(sp, "endDate") ?? defaults.endDate;
  const personA = normalizeBirthInput(parsePerson(sp, "a", DEFAULT_A));
  const personB = normalizeBirthInput(parsePerson(sp, "b", DEFAULT_B));

  const error =
    validateDate(personA) ??
    validateDate(personB) ??
