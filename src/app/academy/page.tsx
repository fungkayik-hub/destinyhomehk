...
import { academyCategoryGroups, getAcademyCategoriesByGroup } from "@/lib/site-config";
      ...
      <div className="py-12 px-4">
        <div className="max-w-4xl mx-auto space-y-12">
          {academyCategoryGroups.map((group) => {
            const categories = getAcademyCategoriesByGroup(group.id);
            if (categories.length === 0) return null;

            return (
              <section key={group.id}>
                <h2 className="font-display text-xl font-bold text-destiny-purple mb-5">
                  {group.label}
                </h2>
